...
def check_alerts():
    ...
    if cpu_usage > 85:
        ...
        if state.cpu_alert_count >= 3:
            ...
            send_alert(
                "CRITICAL: CPU Alert",
                f"CPU has been high for {state.cpu_alert_count} consecutive checks at {cpu_usage}%"
            )
        else:
    ...
    if memory_usage['percent'] > 85:
        ...
        if state.memory_alert_count >= 3:
            ...
            send_alert(
                ...
                f"Memory has been high for {state.memory_alert_count} consecutive checks at {memory_usage['percent']}%"
            )
        else:
    ...
    if disk_usage['percent'] > 85:
        ...
        if state.disk_alert_count >= 3:
            ...
            send_alert(
                "CRITICAL: Disk Alert",
                f"Disk has been high for {state.disk_alert_count} consecutive checks at {disk_usage['percent']}%"
            )
        else:
